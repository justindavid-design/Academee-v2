...
function computeSubmissionStatus(item, submission) {
  // Prefer explicit status values saved on the submission record
  const validStatuses = ['draft', 'submitted', 'late', 'graded', 'returned', 'missing']
  if (submission && submission.status && validStatuses.includes(submission.status)) {
  ...
  // Graded detection (fallback)
  if (submission?.graded_at || submission?.score != null) {
    return 'graded'
  }

  // Submitted detection (fallback)
  if (submission?.submitted_at) {
    // If submitted after due date, mark as late
    if (dueTime && new Date(submission.submitted_at).getTime() > dueTime) {
      return 'late'
    }
    return 'submitted'
  }

  // No submission: if past due, mark missing; otherwise it's assigned/draft
  if (!submission) {
    if (dueTime && dueTime < now) return 'missing'
    return 'assigned'
  }

  // If a submission exists but has no timestamps/content, treat as draft
  if (submission && (submission.content || submission.attachment_url)) {
    return 'draft'
  }
