import { normalizeQuizQuestion } from '../../lib/adaptiveQuizEngine'

export function normalizeQuizQuestions(value) {
  if (Array.isArray(value)) {
    return value
      .map((item, index) => normalizeQuizQuestion(item, index))
      .filter((item) => item && item.question && item.options.length > 1)
  }
...
export function buildQuestionFeedback(question, chosenIndex) {
  ...
  const correctText = question.options?.[question.correct] || question.correctAnswer || 'the correct answer'
