          ...
          <div className="grid gap-4">
            {rows.map((row, index) => (
              <article key={row.title} className="landing-card grid gap-4 bg-surface p-5 sm:grid-cols-[1fr_auto] sm:items-center">
                ...
                <div className={`rounded-lg border border-token px-4 py-3 text-center text-lg font-extrabold text-main ${index === 0 ? 'bg-warning-soft' : index === 1 ? 'bg-info-soft' : 'bg-success-soft'}`}>
                  {row.value}
                </div>
              </article>
            ))}
