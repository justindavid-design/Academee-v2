...
const accessCards = [
  { title: 'High contrast', value: 'AA', tone: 'bg-success-soft' },
  { title: 'Large text', value: '18px+', tone: 'bg-warning-soft' },
  { title: 'Motion control', value: 'Off', tone: 'bg-info-soft' },
]
  ...
  return (
    <header className="relative overflow-hidden bg-app px-4 pb-16 pt-5 md:px-6 md:pb-24">
      <div className={`z-50 mx-auto max-w-6xl transition-all duration-300 ease-out ${isScrolled ? 'fixed inset-x-0 top-0 px-4 pt-3 md:px-6' : 'relative'}`}>
        <div className={`landing-nav mx-auto flex max-w-6xl items-center justify-between gap-4 rounded-lg bg-surface/95 px-4 backdrop-blur-md transition-all duration-300 ease-out md:px-5 ${isScrolled ? 'landing-nav-pop py-2 shadow-card' : 'mt-2 py-3 shadow-none'}`}>
