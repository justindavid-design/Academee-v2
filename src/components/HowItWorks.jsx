          ...
          <div className="grid gap-4">
            {steps.map((step, index) => (
              <article key={step.title} className="grid gap-4 rounded-lg border border-token bg-surface-alt p-5 sm:grid-cols-[auto_1fr]">
                <div className={`flex h-12 w-12 items-center justify-center rounded-lg border border-token text-sm font-bold text-main ${index === 0 ? 'bg-warning-soft' : index === 1 ? 'bg-info-soft' : 'bg-success-soft'}`}>
